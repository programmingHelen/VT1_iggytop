...
def load_data(path: str, max_samples: Optional[int]) -> pd.DataFrame:
    ...
    # extract metadata columns — these are per-cell in adata.obs
    # use .get() fallback so the script works even if columns are missing
    species = adata.obs["antigen_species"].values if "antigen_species" in adata.obs.columns else ["unknown"] * len(adata)
    source  = adata.obs["source"].values          if "source"          in adata.obs.columns else ["unknown"] * len(adata)
    mhc     = adata.obs["MHC_class"].values       if "MHC_class"       in adata.obs.columns else ["unknown"] * len(adata)
    ...
    with ir.get.airr_context(
        ...
        for i in range(len(obs)):
            ...
            rows.append({
                "cdr3b"           : cdr3,
                "epitope"         : epi,
                "antigen_species" : species[i],
                "source"          : source[i],
                "MHC_class"       : mhc[i],
            })

    df = pd.DataFrame(rows).dropna(subset=["cdr3b", "epitope"])
    log.info(f"  Loaded {len(df)} TCR-epitope pairs")
    log.info(f"  Unique species : {df['antigen_species'].nunique()}")
    log.info(f"  Unique sources : {df['source'].nunique()}")
    log.info(f"  MHC classes    : {df['MHC_class'].unique()[:5]}")
...
def generate_negatives(df_pos: pd.DataFrame, ratio: float) -> pd.DataFrame:
    ...
    # negatives only have cdr3b, epitope, label — fill metadata with NaN
    df_neg = pd.DataFrame(negatives)
    for col in ["antigen_species", "source", "MHC_class"]:
        if col in df_pos.columns and col not in df_neg.columns:
            df_neg[col] = "negative"
...
if __name__ == "__main__":
    ...
    # ── MLflow setup ──────────────────────────────────────────────────────────
    # Auto-upgrade DB schema — fixes MlflowException on schema version mismatch
    import subprocess
    subprocess.run(["mlflow", "db", "upgrade", MLFLOW_URI], capture_output=True)
    ...
    with mlflow.start_run():
        ...
        # ── save models: MLflow artifact + local pickle ──────────────────────
        ...
        # ── save shap_data.pkl ────────────────────────────────────────────────
        # Contains everything needed for BOTH notebooks:
        #   results_analysis:    X_test (640-dim), rf_probs, lr_probs, run_id
        #   embedding_analysis:  X_test_tcr (320), X_test_epi (320), df_test_meta
        active_run_id = mlflow.active_run().info.run_id

        X_test_tcr = X_test[:, :320]   # TCR CDR3β embeddings only
        X_test_epi = X_test[:, 320:]   # epitope embeddings only

        # safely select only columns that exist in df
        meta_cols = ["cdr3b", "epitope", "label"]
        for col in ["antigen_species", "source", "MHC_class"]:
            if col in df.columns:
                meta_cols.append(col)
        df_meta_test = df.iloc[idx_test][meta_cols].reset_index(drop=True)

        shap_data = {
            # for results_analysis notebook
            "X_test"      : X_test,
            "y_test"      : y_test,
            ...
            # for embedding_analysis notebook
            "X_test_tcr"  : X_test_tcr,
            "X_test_epi"  : X_test_epi,
            # metadata aligned to test set rows
            "test_indices": idx_test,
            "df_test_meta": df_meta_test,
        }

        with open("outputs/shap_data.pkl", "wb") as f:
            pickle.dump(shap_data, f)
        mlflow.log_artifact("outputs/shap_data.pkl")
        log.info("✓ Saved shap_data.pkl")
        ...
        log.info("✓ Saved model_metrics_summary.csv")
